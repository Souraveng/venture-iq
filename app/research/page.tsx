...
const segments = [
  { name: "Primary B2B Segment", value: 38, color: "#daf264" },
  { name: "SME / Mid-Market", value: 29, color: "#c8e84a" },
  { name: "Enterprise Contracts", value: 21, color: "#7ab010" },
  { name: "Consumer Segment", value: 12, color: "#4a7a00" },
];

const geographies = [
  { region: "Domestic (Primary Hub)", opportunity: 92 },
  { region: "Regional Scope",  opportunity: 78 },
  { region: "National Market",     opportunity: 65 },
  { region: "Global Expansion",          opportunity: 55 },
];
...
export default function ResearchPage() {
  ...
  const dynamicSegments = Array.isArray(activeProject?.marketIntel?.customerSegments) && activeProject.marketIntel.customerSegments.length > 0
    ? activeProject.marketIntel.customerSegments.map((s: any, idx: number) => {
        const colors = ["#daf264", "#c8e84a", "#7ab010", "#4a7a00"];
        return {
          name: s.segmentName || `Segment ${idx + 1}`,
          value: parseFloat(s.sizeEstimate?.replace(/[^0-9.]/g, "")) || (idx === 0 ? 38 : idx === 1 ? 29 : idx === 2 ? 21 : 12),
          color: colors[idx % colors.length]
        };
      })
    : segments;

  const dynamicGeographies = activeProject?.marketIntel?.sam?.geographicScope
    ? [
        { region: activeProject.marketIntel.sam.geographicScope + " (Primary)", opportunity: 92 },
        { region: "Secondary Scope",  opportunity: 78 },
        { region: "National Market",  opportunity: 65 },
        { region: "Global Expansion", opportunity: 55 },
      ]
    : geographies;

  const dynamicInsights = Array.isArray(activeProject?.marketIntel?.marketTrends) && activeProject.marketIntel.marketTrends.length > 0
    ? activeProject.marketIntel.marketTrends.slice(0, 2).map((trendItem: any) => ({
        title: trendItem.trend || "Market Trend",
        body: trendItem.description || "",
        icon: "◎"
      }))
    : [
        { title: t("keyTrend") !== "keyTrend" ? t("keyTrend") : "Key Trend", body: "The target sector is experiencing rapid expansion driven by digitization and operational efficiency demands.", icon: "↑" },
        { title: t("opportunityWindow") !== "opportunityWindow" ? t("opportunityWindow") : "Opportunity Window", body: "Early-mover advantage is available for software-native providers addressing current workflow gaps.", icon: "◎" },
      ];
      ...
      <div className="p-8 space-y-6">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-medium px-2 py-0.5 rounded-full"
                style={{ background: "rgba(218, 242, 100, 0.1)", color: ACC }}>{t("marketResearchAgent")} · {t("done") !== "done" ? t("done") : "Done"}</span>
            </div>
            <h1 className="text-2xl font-bold text-white">{t("marketAnalysisReport")}</h1>
            <p className="text-sm mt-1" style={{ color: "var(--muted-fg)" }}>
              {activeProject?.name || "No Active Venture"} · {activeProject?.description ? activeProject.description.slice(0, 60) + "..." : "India Market"} · Generated by AI
            </p>
          </div>
        ...
        {activeProject?.finalReport?.summary && (
          <PremiumGate>
            <div className="rounded-xl p-5" style={{ background: "rgba(218, 242, 100, 0.05)", border: "1px solid rgba(218, 242, 100, 0.2)" }}>
              <h3 className="font-semibold text-sm text-white mb-2">{t("executiveSummary")}</h3>
        ...
        {activeProject?.marketIntel?.rawResearch && (
          <div className="rounded-xl p-5" style={{ background: "var(--card-bg)", border: "1px solid var(--card-border)" }}>
            <h3 className="font-semibold text-sm text-white mb-2">{t("liveWebSearch")}</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="rounded-xl p-5" style={{ background: "var(--card-bg)", border: "1px solid var(--card-border)" }}>
            <h3 className="font-semibold text-sm text-white mb-1">{t("marketGrowthTrajectory")}</h3>
            <p className="text-xs mb-4" style={{ color: "var(--muted-fg)" }}>{t("globalMarketSize") !== "globalMarketSize" ? t("globalMarketSize") : "Global target market size ($T) · 2024–2030"}</p>
          ...
          <div className="rounded-xl p-5" style={{ background: "var(--card-bg)", border: "1px solid var(--card-border)" }}>
            <h3 className="font-semibold text-sm text-white mb-1">{t("customerSegments")}</h3>
            <p className="text-xs mb-4" style={{ color: "var(--muted-fg)" }}>{t("marketShareBySegment") !== "marketShareBySegment" ? t("marketShareBySegment") : "Market share by segment"}</p>
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={dynamicSegments} layout="vertical" margin={{ left: -8 }}>
                ...
                <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                  {dynamicSegments.map((s, i) => <Cell key={i} fill={s.color} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
        ...
        <div className="rounded-xl p-5" style={{ background: "var(--card-bg)", border: "1px solid var(--card-border)" }}>
          <h3 className="font-semibold text-sm text-white mb-4">{t("geographicOpportunityScore")}</h3>
          <div className="space-y-3">
            {dynamicGeographies.map((g) => (
              <div key={g.region}>
        ...
        {activeProject?.researchPlan && Array.isArray(activeProject.researchPlan) && activeProject.researchPlan.length > 0 && (
          <div className="rounded-xl p-5" style={{ background: "var(--card-bg)", border: "1px solid var(--card-border)" }}>
            <h3 className="font-semibold text-sm text-white mb-3 flex items-center gap-2">
              <span style={{ color: ACC }}>◈</span> {t("researchPlan")} ({activeProject.researchPlan.length})
            </h3>
        ...
        {activeProject?.evidence && Array.isArray(activeProject.evidence) && activeProject.evidence.length > 0 && (
          <div className="rounded-xl p-5" style={{ background: "var(--card-bg)", border: "1px solid var(--card-border)" }}>
            <h3 className="font-semibold text-sm text-white mb-3 flex items-center gap-2">
              <span style={{ color: ACC }}>↗</span> {t("sourcesCitations")} ({activeProject.evidence.length})
            </h3>
